...
type Block struct {
	...
	Nonce            itctypes.BlockNonce `json:"nonce"`
...
// Note that all txs on intelchain are replay protected (post EIP155 epoch).
func NewTransaction(
...
func newBlock(b *types.Block) *Block {
	...
	return &Block{
		...
		Nonce:            itctypes.BlockNonce{}, // Legacy comment from itc -> eth RPC porting: "Remove this because we don't have it in our header"
		MixHash:          head.MixDigest(),
		UncleHash:        itctypes.CalcUncleHash(b.Uncles()),
		LogsBloom:        head.Bloom(),
		StateRoot:        head.Root(),
		Difficulty:       (*hexutil.Big)(big.NewInt(0)), // Legacy comment from itc -> eth RPC porting: "Remove this because we don't have it in our header"
