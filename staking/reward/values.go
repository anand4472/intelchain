...
// in the pre-staking era (epoch < staking epoch) in INTELLI.
//
...
var (
	totalPreStakingNetworkRewardsInIntelli = map[shardingconfig.NetworkID][]*big.Int{
...
// getTotalPreStakingNetworkRewards in INTELLI for given NetworkID
func getTotalPreStakingNetworkRewards(id shardingconfig.NetworkID) *big.Int {
	totalRewards := big.NewInt(0)
	if allRewards, ok := totalPreStakingNetworkRewardsInIntelli[id]; ok {
...
func SetTotalInitialTokens(initTokensAsIntelli *big.Int) {
	TotalInitialTokens = numeric.NewDecFromBigIntWithPrec(initTokensAsIntelli, 18)
}
