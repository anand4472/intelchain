...
import (
	...
	//cmdintelchain "github.com/zennittians/intelchain/cmd/intelchain"
	...
	bootnode "github.com/zennittians/intelchain/node/boot"
...
func printVersion(me string) {
	fmt.Fprintf(os.Stderr, "Intelchain (C) 2024. %v, version %v-%v (%v %v)\n", path.Base(me), version, commit, builtBy, builtAt)
}
...
func main() {
	...
	intelchainConfigs.VersionMetaData = append(intelchainConfigs.VersionMetaData, path.Base(os.Args[0]), version, commit, builtBy, builtAt)
	nodeConfigs.SetVersion(intelchainConfigs.GetIntelchainVersion())
	nodeConfigs.SetPeerID(host.GetID())
	hc := intelchainConfigs.GetDefaultConfigCopy()
