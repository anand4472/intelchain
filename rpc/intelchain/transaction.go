...
import (
	...
	"github.com/intelchain-itc/intelchain/accounts/abi"
	...
	eth "github.com/intelchain-itc/intelchain/rpc/intelchain/eth"
	v1 "github.com/intelchain-itc/intelchain/rpc/intelchain/v1"
	v2 "github.com/intelchain-itc/intelchain/rpc/intelchain/v2"
...
func (s *PublicTransactionService) EstimateGas(
	...
	gas, err := EstimateGas(ctx, s.itc, args, bNrOrHash, nil)
	if err != nil {
		return 0, err
	}
...
func (s *PublicTransactionService) GetTransactionByHash(
	...
	if tx == nil {
		...
		if tx := s.itc.TxPool.Get(hash); tx != nil {
	...
	block, err := s.itc.GetHeader(ctx, blockHash)
	if err != nil {
		utils.Logger().Debug().
			Err(err).
			Msgf("%v error at %v", LogTag, "GetTransactionByHash")
...
func EstimateGas(ctx context.Context, itc *itc.Intelchain, args CallArgs, blockNrOrHash rpc.BlockNumberOrHash, gasCap *big.Int) (uint64, error) {
	...
	if args.Gas != nil && uint64(*args.Gas) >= params.TxGas {
		...
		blk, err := itc.BlockByNumberOrHash(ctx, blockNrOrHash)
		if err != nil {
			return 0, err
		}
		hi = blk.GasLimit()
	...
	if args.GasPrice != nil && args.GasPrice.ToInt().BitLen() != 0 {
		state, _, err := itc.StateAndHeaderByNumberOrHash(ctx, blockNrOrHash)
		if err != nil {
			return 0, err
		}
	...
	executable := func(gas uint64) (bool, *core.ExecutionResult, error) {
		...
		result, err := DoEVMCall(ctx, itc, args, blockNrOrHash, 0)
		if err != nil {
