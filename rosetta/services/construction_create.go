...
import (
	...
	common2 "github.com/intelchain-itc/intelchain/internal/common"
...
) (*WrappedTransaction, itcTypes.PoolTransaction, *types.Error) {
...
func (s *ConstructAPI) ConstructionPayloads(
	...
	if metadata.Transaction.FromShardID != nil && *metadata.Transaction.FromShardID != s.itc.ShardID {
		return nil, common.NewError(common.InvalidTransactionConstructionError, map[string]interface{}{
			"message": fmt.Sprintf("transaction is for shard %v != shard %v",
				*metadata.Transaction.FromShardID, s.itc.ShardID,
			),
		})
	}

	unsignedTx, rosettaError := ConstructTransaction(components, metadata, s.itc.ShardID)
	if rosettaError != nil {
		return nil, rosettaError
	}
...
func (s *ConstructAPI) getSigningPayload(
	tx itcTypes.PoolTransaction, senderAccountID *types.AccountIdentifier,
	...
	case *itcTypes.Transaction:
		payload.Bytes = s.signer.Hash(tx.(*itcTypes.Transaction)).Bytes()
	default:
		return nil, common.NewError(common.CatchAllError, map[string]interface{}{
